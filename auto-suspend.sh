...
download_autosuspend() {
    ...
    unzip -tq "$TMP_DIR/expdate.zip" \
        || die "expdate.zip rusak atau bukan ZIP yang valid."
    ...
    [[ -d "$TMP_DIR/extracted/database" ]] \
        || die "Folder database tidak ditemukan di expdate.zip."

    log "Menyalin isi expdate.zip ke Pterodactyl..."

    cp -a "$TMP_DIR/extracted/." "$PTERODACTYL/"
...
TARGET_NEW="resources/views/admin/servers/new.blade.php"
  ...
  if [ -f "$TARGET_NEW" ] && ! grep -q "name=\"exp_date\"" "$TARGET_NEW"; then
...
\
    ...
    print_info "Field Expiration Date sudah ada atau file Create Server tidak ditemukan."
  fi
