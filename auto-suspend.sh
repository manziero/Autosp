...
download_autosuspend() {
    ...
    # ZIP yang digunakan hanya berisi database/migrations.
...
patch_kernel() {
    ...
    if ! grep -Fq 'use Pterodactyl\Models\Server;' "$file"; then
        if grep -Fq 'use Ramsey\Uuid\Uuid;' "$file"; then
            ...
            log "Import UUID tidak ditemukan, mencoba menambahkan Server model."
...
if "Server::whereNotNull('exp_date')" not in text:
    if marker not in text:
        raise SystemExit(
            "Scheduler Pterodactyl tidak ditemukan."
        )
            ...
            if grep -Fq "Character limits:" "$file"; then
                ...
                sed -i \
                    ...
                    <div class="form-group">\
                        ...
                        <p class="text-muted small">Server akan kadaluarsa pada tanggal yang dipilih. Kosongkan untuk server permanen.<\/p>\
                    <\/div>|
                        }
                    }' \
                    "$file"
...
verify() {
    ...
    if grep -Fq \
        ...
        log "PERINGATAN: Scheduler Auto Suspend tidak ditemukan."
    fi
...
main() {
    ...
    log "Expiration Date tersedia di Create Server jika"
    log "struktur form panel cocok dengan patch."
    
    log "Scheduler Auto Suspend dijalankan setiap hari pukul 23:55."
}
